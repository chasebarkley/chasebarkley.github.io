function appsFunction() {
var x = document.getElementById("apps");
if (x.style.display === "none") {
        x.style.display = "block";
    } else {
        x.style.display = "none";
    }
}
function plattersFunction() {
var x = document.getElementById("chkPlat");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function steaksFunction() {
var x = document.getElementById("steak");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function seaFunction() {
var x = document.getElementById("sea");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function specialFunction() {
var x = document.getElementById("special");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function burgerFunction() {
var x = document.getElementById("burgers");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function sandFunction() {
var x = document.getElementById("sand");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function saladFunction() {
var x = document.getElementById("salad");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function lunchFunction() {
var x = document.getElementById("lunch");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function sideFunction() {
var x = document.getElementById("sides");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function psideFunction() {
var x = document.getElementById("psides");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function comboFunction() {
var x = document.getElementById("combo");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function wingFunction() {
var x = document.getElementById("wings");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function dessertFunction() {
var x = document.getElementById("desserts");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
function familyFunction() {
    var x = document.getElementById("family");
    if (x.style.display === "none") {
        x.style.display = "block";
    } else {
        x.style.display = "none";
    }
    }
function kidFunction() {
var x = document.getElementById("kids");
if (x.style.display === "none") {
    x.style.display = "block";
} else {
    x.style.display = "none";
}
}
